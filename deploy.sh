...
startup() {
  ...
  helm upgrade --install $INFRA_NAME test-497-infrastructure \
     --values test-497-infrastructure/values.yaml \
     --values test-497-infrastructure/values-dev.yaml
...
deploy() {
  ...
  if is_app_running; then
    echo "test-497 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-497 from branch '$branch'..."
